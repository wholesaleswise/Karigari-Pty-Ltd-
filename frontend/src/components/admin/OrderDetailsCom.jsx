...
const OrderDetailsCom = ({ order, id }) => {
  ...
  const [updateOrderStatus, { isLoading: updateLoading }] =
    useUpdateOrderStatusMutation();
  const [updatePaymentStatus, { isLoading: updatePaymentLoading }] =
    useUpdateOrderPaymentStatusMutation();
  ...
  const handleOrderStatusChange = async (id) => {
    try {
      const response = await updateOrderStatus({ newStatus, id }).unwrap();
    ...
    <>
      <div className="container mx-auto p-6 md:p-8 space-y-6 bg-white rounded-md shadow-md">
        <div className="flex justify-end gap-4 print:hidden">
          <Button
            onClick={handleDownloadPDF}
            className="bg-green-500 hover:bg-green-600 text-white px-4 py-2 text-sm rounded-md flex items-center gap-2"
          >
              ...
              <table className="min-w-full table-auto rounded-md shadow-sm">
                ...
                <tbody>
                  {productDetails.map((item) => {
                    ...
                    return (
                      <tr key={item._id} className="border-b border-gray-200">
                        <td className="px-4 py-3 flex items-center gap-2 min-w-60 max-w-80">
                          {productImageUrls?.[0] && (
                            <img
                              src={productImageUrls[0]}
                              alt={productName}
          ...
          <div className="bg-white rounded-lg p-6 w-96 shadow-lg">
            ...
            <div className="mt-6 flex justify-end gap-3">
              ...
              <Button
                ...
                onClick={() => handleOrderStatusChange(id)}
              >
                Save Status
              </Button>
            </div>
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-gray-600 bg-opacity-50 z-50">
          <div className="bg-white rounded-lg p-6 w-96 shadow-lg">
            ...
            <input
              type="number"
              min="0"
              max={paymentDetails.remainingAmount || totalAmount}
              placeholder="Enter paid amount"
              value={paymentAmount}
              onChange={(e) => setPaymentAmount(e.target.value)}
              className="w-full border border-gray-300 rounded-md px-3 py-1 mb-4 "
            />
            ...
            >
              <SelectTrigger className="w-full rounded-md border border-gray-300 mb-4">
            ...
            <div className="mt-6 flex justify-end gap-3">
              ...
              <Button
                disabled={
                  !paymentAmount || !paymentStatus || updatePaymentLoading
                }
                onClick={() => handlePaymentStatusChange(id)}
              >
                Save Payment
              </Button>
            </div>
